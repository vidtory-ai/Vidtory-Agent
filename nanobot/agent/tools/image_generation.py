...
class ImageGenerationTool(Tool, ContextAware):
    ...
    def description(self) -> str:
        return (
            ...
            "4. EDIT/REVISION RULE: When a user asks to edit, add text, fix text, or modify an existing image (e.g. 'thêm chữ...', 'sửa lỗi...'), "
            "you MUST call this generate_image tool to actually perform the edit. "
            "DO NOT just reply with text confirming the change without calling the tool.\n\n"
    ...
    def _find_last_actual_generated_image(self) -> str | None:
        """Find the last image actually generated by the assistant or tool response in the session.
        
        Does not include user-uploaded images or current request media.
        """
        ctx = _image_gen_request_ctx.get()
            ...
            for msg in reversed(session.messages):
                ...
                if role == "tool" and msg.get("name") == "generate_image" and content:
                    try:
                        ...
                        if artifacts and isinstance(artifacts, list):
                            ...
                            if path:
                                logger.info("Found last actual generated image from tool response: {}", path)
                                return path
                    except Exception:
                        pass
                ...
                if role == "assistant" and msg.get("media"):
                    ...
                    if isinstance(h_media, list) and h_media:
                        logger.info("Found last actual generated image from assistant media: {}", h_media[-1])
                        return h_media[-1]
        except Exception as exc:
            logger.warning("Error finding last actual generated image: {}", exc)
    ...
    def _merge_revision_references(
        ...
        # 1. Prioritize newly uploaded request media in the current message
        if request_media:
            logger.info("Using newly uploaded request media as references: {}", request_media)
            return merged

        # Find the last actually generated image from history
        last_generated = self._find_last_actual_generated_image()
        ...
        # 3. Case: Revision / adjustment prompt
        if _is_revision_prompt(prompt):
            # If it is a revision/adjustment prompt, and we have a generated image,
            # we should use it as the reference image, avoiding restoring original user images.
            if last_generated:
                logger.info(
                    "Revision prompt: using last actual generated image as reference: {}",
                    last_generated,
                )
                return [last_generated]

            # If no generated image exists, but the user explicitly references the latest image
            # (e.g. "từ ảnh trên..."), fall back to finding any last image (including user uploads).
