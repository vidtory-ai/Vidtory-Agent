...
# Cache for dynamic clients, keyed by (api_key, base_url, tuple_of_sorted_headers)
_dynamic_client_cache: dict[tuple[str, str | None, tuple[tuple[str, str], ...]], Any] = {}
_dynamic_client_lock = asyncio.Lock()
    ...
    async def _ensure_client(self):
        ...
        if user_key:
            cache_key = (
                ...
                tuple(sorted(self._default_headers.items())),
            )
            async with _dynamic_client_lock:
                if cache_key not in _dynamic_client_cache:
                    import httpx
                    timeout_s = _openai_compat_timeout_s()
                    http_client: httpx.AsyncClient | None = None
                    if self._is_local:
                    ...
                    _dynamic_client_cache[cache_key] = AsyncOpenAI(
                        api_key=user_key,
                        base_url=self._effective_base,
                        default_headers=self._default_headers,
                        max_retries=0,
                ...
                return _dynamic_client_cache[cache_key]
