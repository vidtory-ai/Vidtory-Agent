...
class VidtoryImageGenerationClient(ImageGenerationProvider):
    """Async client for Vidtory B2B Image Generation."""
    ...
    async def generate(
        ...
        *,
        ...
        vidtory_ar = ar_map.get(aspect_ratio or "1:1", "IMAGE_ASPECT_RATIO_SQUARE")
        ...
        url = f"{self.api_base}/generative-core/image"
        client = self._client or httpx.AsyncClient(timeout=self.timeout)
        try:
            # Initiate job
            ...
            try:
                ...
                detail = response.text[:500]
                raise ImageGenerationError(f"Vidtory image generation initiation failed: {detail}") from exc

            init_data = response.json()
            if not init_data.get("success"):
                raise ImageGenerationError(f"Vidtory image generation failed: {init_data.get('message')}")

            job_data = init_data.get("data") or {}
            job_id = job_data.get("generationHistoryId")
            if not job_id:
                raise ImageGenerationError("Vidtory did not return a generationHistoryId")
            ...
            while True:
                ...
                try:
                    ...
                    raise ImageGenerationError(f"Vidtory job status polling failed: {detail}") from exc

                status_payload = poll_resp.json()
                if not status_payload.get("success"):
                    raise ImageGenerationError(f"Vidtory job status check failed: {status_payload.get('message')}")
                ...
                if status == "COMPLETED":
                    ...
                    raise ImageGenerationError(f"Vidtory job execution failed: {err_msg}")
        finally:
            if self._client is None:
                await client.aclose()
...
register_image_gen_provider(VidtoryImageGenerationClient)
