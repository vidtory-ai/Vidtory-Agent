"""CLI commands for Vidtory-Agent."""
...
app = typer.Typer(
    ...
    help=f"{__logo__} Vidtory-Agent - Personal AI Assistant",
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} Vidtory-Agent[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} Vidtory-Agent[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} Vidtory-Agent v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """Vidtory-Agent - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize Vidtory-Agent configuration and workspace."""
    ...
    if wizard:
        ...
        try:
            ...
            console.print("[yellow]Please run 'vidtoryagent onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'vidtoryagent agent -m "Hello!"'
    gateway_cmd = "vidtoryagent gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} Vidtory-Agent is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/vidtory-ai/Vidtory-Agent#-chat-apps[/dim]"
    )
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show Vidtory-Agent runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'vidtoryagent[api]'[/red]")
        raise typer.Exit(1)
...
def gateway(
    ...
    """Start the Vidtory-Agent gateway."""
    if verbose:
...
def _run_gateway(
    ...
    *,
    ...
    console.print(f"{__logo__} Starting Vidtory-Agent gateway version {__version__} on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show Vidtory-Agent runtime logs during chat"),
):
...
def status():
    ...
    console.print(f"{__logo__} Vidtory-Agent Status\n")
